...
def _review(world, result, owners) -> dict:
    ...
    on the map: the join grammar describes E3M1's street and nothing inside
    its buildings.
    """
    kinds = result["kinds"]["kinds"]
    census = result["census"]
    tree = Tree(len(world.sectors), "E3M1 -- joins the writer's table describes")
    ...
    questions = [
        {"node": "row:road|pavement|b_above",
         "question": ("The reader recovers 11 road|pavement records and all "
                      ...
                      "`facade stone` as 400 and E3M1's 3 road|end_wall "
                      "records all wear 414. Is 400 a campaign value or "
                      "Gravesend's own choice?"),
         "recommended_default": ("treat 400 as our CHOICE and 414 as E3M1's, "
                                 "and mine the class over the campaign before "
                                 "either is called a default; the kerb row "
                                 "needs no change"),
    ...
    return write_pack(3, tree, "E3M1 layer 3: joins", questions)
...
def main() -> int:
    ...
    print(f"E3M1 joins: {stats['two_sided_records']} two-sided records, "
    ...
    if 10 in solid and 11 in solid:
        out.append({
            ...
            "reconciled": f"the ROW is still attested -- "
                          ...
                          f"pavement|pavement records exist in E3M1, between "
