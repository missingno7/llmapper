...
def _review(world, places, names) -> dict:
    tree = Tree(len(world.sectors), "E3M1 -- names, and the refusals")
    named = places["named"]
    if named:
    ...
    questions = [
        ...
        {"node": (f"named:{named[0]['space']}" if named else "level"),
         "question": (f"A mechanism is named by the prefix of the lesson "
                      f"files teaching its type and shape -- "
                      f"{names['named'][0]['name'].upper()}-* for "
                      f"{names['named'][0]['sentence']}, on "
                      f"{names['named'][0]['basis']}. Is the curriculum's own "
                      f"file naming a fair source of function names?"),
         "recommended_default": ("yes, with the share reported: it is the "
                                 "campaign's vocabulary rather than ours, and "
                                 "where no prefix holds 60% the reader "
                                 "refuses instead of picking"),
         "evidence": "references/intent.json: mechanisms.named"},
    ]
    return write_pack(8, tree, "E3M1 layer 8: intent", questions)
