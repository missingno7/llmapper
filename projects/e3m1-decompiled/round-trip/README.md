...
`E3M1.MAP` here is the original rebuilt from `../facts/`: every field a
...
`maps/blood/campaign/E3M1.MAP`, so committing it would redistribute a
commercial map — the rule `maps/` itself is under. One command makes it:

```bash
PYTHONPATH=. python -m tools.round_trip maps/blood/campaign/E3M1.MAP \
    projects/e3m1-decompiled/facts \
    -o projects/e3m1-decompiled/round-trip/E3M1.MAP \
    --report projects/e3m1-decompiled/round-trip/E3M1.md
```

`E3M1.md` and `E3M1.json` ARE committed: they are the measurement, and they
