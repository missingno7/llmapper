...
from bloodmap.prefab import alcove_run, breakable, sprite_bridge
...
from bloodmap.surfaces import MATERIALS, check as check_materials, material
from bloodmap.planar_layout import PlanarLayout
from bloodmap.slope import SlopeSpec
from bloodmap.switches import exit_switch
...
#: How high a switch a player *presses* hangs.
#:
#: Not a rescale of the old number, though the old number was right in absolute
#: terms: 2.18 in the pre-correction unit was 12,277 z, and the campaign's
#: pressed switches sit at 13,398. Taken from the corpus instead --
#: `knowledge/blood/design/switches-v1.json`, 453 switches carrying
#: `trigger_push`, median 0.79 humans above the floor with a q3 of 0.79 and a
#: p95 of 0.84. The eye is at 0.83, so a pressed switch is at eye level or a
#: little below, which is what "in reach" means for a hitscan from the eye.
#:
#: A switch *shot* rather than pressed is a different thing and lives higher --
#: median 1.93, q3 3.26 -- and wears a different tile. See `bloodmap.switches`.
SWITCH_HEIGHT = 0.79
SWITCH_OFFSET = 0.12
...
#: Tile 266 is the campaign's commonest masked tile by a factor of four, and it
#: is glass: a pale pane with a highlight streak across it. It is also Blood's
#: commonest *breakable* wall by a distance -- 247 of the 406 walls in the
#: campaign and Death Wish that carry `kWallGib`.
#:
#: This was read here as frosted glass on the grounds that 0% of its pixels are
#: transparent, and that is true and beside the point. Transparency is a
#: property of the tile; **translucency is cstat bit 2**, which blends the whole
#: surface through the game's TLU table however opaque its pixels are. 1,461
#: walls across the corpus set it. A pane with no transparent pixels and bit 2
#: set is exactly a window you can see through -- and with wall type 511 it is
#: one you can put a shotgun through.
WINDOW_GLASS = 266

#: Glass: blocking, translucent, masked, hitscan. The bit that was missing is
#: the 2.
GLASS_CSTAT = 1 | 2 | 16 | 64
...
def water_garden(layout):
    ...
    garden_steps = staircase(
        ...
        # Under the porch's eave rather than open to the sky, for the same
        # reason as the ascent: an open flight carries its sky down with it.
    ...
    layout.add_region("region:cascade_shelf", SHELF, role="exterior",
                      ...
                      **material("hedge"), declared_zero_exit=True,
    ...
    # Roofed, not open. The breach joins the graveyard, whose sky is 20.4 player
    # heights up, to the garden, whose sky is 12 -- and two open sectors that
    # touch hold their sky at one height in 94% of the campaign. Left open, the
    # difference is drawn as a band of wall hanging over the gap. The masonry
    # that did not fall is the division.
    layout.add_region("region:garden_breach", R(52, -64, 54, -60), role="gateway",
                      ceiling_z=COURT - tex(6), floor_z=BREACH_FLOOR,
                      **material("rubble_roofed"),
    ...
    # Each heap takes the sky of the yard it lies in, not the sky of the wall it
    # fell out of. Both were given the graveyard's, so the garden-side heap held
    # its sky 10.2 player heights above the lawn's and the difference was drawn
    # as a band over the rubble.
    for tag, host, box, ground, sky in (
        ("grave", "region:graveyard", R(49.5, -63.5, 51.5, -60.5), COURT, GRAVEYARD_SKY),
        ("garden", "region:garden_court", R(54.5, -63.5, 56.5, -60.5), GARDEN_LAWN, GARDEN_SKY),
    ):
        rid = "region:breach_spill_%s" % tag
        layout.carve_hole(host, box)
        layout.add_region(rid, box, role="detail",
                          ceiling_z=sky, floor_z=ground - 2048,
                          **material("rubble"),
...
# =====================================================================
#
# E1M4 is a carnival, and it is worth being precise about what makes it one,
# because almost none of it is architecture. Its 398 sectors are mostly ordinary
# rooms. What carries the theme is the dressing and the palette:
#
# * a poster reading HE'S UNCONTROLLABLE! JO-JO THE IDIOT CIRCUS BOY (981) and a
#   ride sign reading Happy-Go-Pukey! (1259);
# * strips of marquee bulbs (77) drawn wall-aligned at pal 14;
# * balloons (1115) -- the same tile in four different palettes, 4, 11, 12 and
#   13, which is the whole trick: one sprite, four colours;
# * mime dummies (2400), waste barrels (505), fairground lamps (640);
# * and a shooting gallery: rubber ducks (852) and grinning faces (853) on
#   sliding sectors, all wired to one channel, retriggering in both directions
#   so they never stop, with their shade animated on wave 5 at amplitude -46 so
#   the targets flash while they travel.
#
# Nearly every one of those props is `kThingObjectGib` (type 416) -- a breakable.
# The carnival is a thing you shoot.
#
# What is *not* copied is E1M4's rotating and sliding sector types, 617 and 616.
# Those translate a sector's own walls, and a wall pair has to stay coincident
# with its neighbour's, so an island in a yard cannot use them. The marked
# variant this project already builds gates with -- 614, where the sprites carry
# the motion and the geometry stays put -- is what a shooting gallery wants
# anyway: the targets move, the booth does not.

CARNIVAL_FLOOR = COURT
CARNIVAL_MARQUEE = 77           # a strip of bulbs, drawn at pal 14
CARNIVAL_BALLOON = 1115
BALLOON_PALETTES = (4, 11, 12, 13)
CARNIVAL_POSTER = 981           # JO-JO THE IDIOT CIRCUS BOY
CARNIVAL_RIDE_SIGN = 1259       # Happy-Go-Pukey!
CARNIVAL_LAMP = 640
CARNIVAL_BARREL = 505
CARNIVAL_MIME = 2400
TARGET_DUCK = 852
...
#: The two bits that make a sprite ride a marked sector: 8192 goes one way,
#: 16384 the other. E1M4's targets carry 8192.
CARRIED = 8192

#: The gallery's targets flash as they travel. E1M4 sets wave 5 at amplitude -46
#: with frequency 15 on every one of its three target sectors -- a much larger
#: swing than a torch's, because this is a bulb behind a target and not a flame.
TARGET_WAVE = 5
TARGET_AMPLITUDE = -46
TARGET_FREQUENCY = 15


def dark_carnival(layout):
    """A travelling show that set up in the monastery and never left.

    THE BRIEF, and the numbers it is held to
    ----------------------------------------

    The first version of this was a twenty-by-twenty yard with two mimes and
    eight balloons in it, reached over a nine-unit bridge. It was not a carnival,
    it was a room with carnival props in it, and the difference is size.

    E1M4 measured: **387 playable sectors, 426 by 253 player widths, 23,841
    player widths squared of floor, and a biggest room of 2,181 at 15.3 player
    heights.** DWE1M1 is three times that again, with a room of 4,989 at 60.9.
    The monastery as a whole is 6,789 -- less than a third of E1M4 -- and its
    biggest room was 1,012.

    So the carnival is built to be **the biggest thing in the level**: a midway
    of about 1,100 player widths squared under twenty player heights of sky,
    which is E1M4's own ceiling for its largest space. Everything else follows
    from wanting that to feel large rather than merely be large:

    * **the approach is long.** A seventeen-unit bridge over a pit eighty player
      heights deep, with nothing on either side of it. The walk is the point;
      you should have time to look down.
    * **the midway is one space, not several.** Sight lines the length of it,
      with the attractions arranged along the edges rather than partitioning it.
    * **there is something tall in it.** A marquee on the midway's west side,
      floored a step up and roofed at twelve, so the sky reads as sky by
      contrast with something that is not.
    * **the far end is visible from the near end and worth walking to.** The
      shooting gallery sits in a recess in the north wall, lit and moving,
      directly opposite where you come in.

    The vocabulary is E1M4's, measured rather than guessed: the JO-JO poster
    (981) and the Happy-Go-Pukey sign (1259), marquee bulb strips (77) at pal
    14, balloons (1115) in four palettes, mime dummies (2400), waste barrels
    (505), fairground lamps (640), and rubber ducks and grinning faces (852,
    853) on a marked slide, flashing on wave 5 at amplitude -46. Nearly all of
    them are `kThingObjectGib` -- the carnival is a thing you shoot.
    """

    # -- the way in ----------------------------------------------------------
    layout.add_region("region:carnival_walk", R(6, -24, 10, -14), role="gateway",
                      ceiling_z=COURT - tex(4), floor_z=COURT,
                      **material("cloister"),
                      intent={"purpose": "passage north from the refectory towards "
                                         "the carnival",
                              "classification": "OPTIONAL"})
    layout.add_connection("connection:refectory_carnival", "region:refectory",
                          "region:carnival_walk",
                          a1=P(6, -14), a2=P(10, -14), min_width=1536)

    # -- the pit -------------------------------------------------------------
    #
    # Death Wish crosses chasms on blocking floor-aligned sprites -- DWE1M1 lays
    # them over a floor eighty-three player heights down. This is that, and the
    # span is seventeen units rather than nine because a bridge you are across
    # before you have looked down is a doorstep.
    PIT_FLOOR = COURT + 80 * PH
    layout.add_region("region:carnival_pit", R(-2, -42, 18, -24), role="exterior",
                      ceiling_z=COURT - tex(20), floor_z=PIT_FLOOR,
                      **material("courtyard"), declared_zero_exit=True,
                      intent={"purpose": "the chasm the carnival stands beyond, "
                                         "crossed on sprites and on nothing else",
                              "classification": "OPTIONAL"})
    layout.add_connection("connection:carnival_walk_pit", "region:carnival_walk",
                          "region:carnival_pit", a1=P(6, -24), a2=P(10, -24),
                          min_width=1536)
    sprite_bridge(layout, "carnival_bridge", "region:carnival_pit",
                  start=P(8, -24.5), end=P(8, -41.5), z=COURT)

    # -- the midway ----------------------------------------------------------
    #
    # 44 by 26 units, which is 1,144 player widths squared -- the biggest space
    # in this level by a factor of two, and half of E1M4's biggest. Notched at
    # the north for the shooting gallery, holed in the west for the marquee.
    MIDWAY = poly((-26, -68), (3, -68), (3, -64), (17, -64), (17, -68),
                  (18, -68), (18, -42), (-26, -42))
    layout.add_region("region:carnival_midway", MIDWAY, role="exterior",
                      ceiling_z=COURT - tex(20), floor_z=CARNIVAL_FLOOR,
                      **material("courtyard"),
                      intent={"purpose": "the midway: the largest space in the "
                                         "level, and the reason the bridge is long",
                              "classification": "OPTIONAL"})
    layout.add_connection("connection:pit_midway", "region:carnival_pit",
                          "region:carnival_midway", a1=P(6, -42), a2=P(10, -42),
                          min_width=1536)
    for tag, a1, a2 in (("w", P(-2, -42), P(6, -42)), ("e", P(10, -42), P(18, -42))):
        layout.add_partition("partition:carnival_lip_%s" % tag, "region:carnival_pit",
                             "region:carnival_midway", role="blocked_portal",
                             opaque=True, a1=a1, a2=a2)

    # -- the marquee ---------------------------------------------------------
    #
    # An octagon on poles: floored a step up, roofed at twelve player heights
    # while the midway is open to twenty. The sky reads as sky because there is
    # something next to it that is not.
    BIGTOP = poly((-19, -62), (-9, -62), (-6, -59), (-6, -53), (-9, -50),
                  (-19, -50), (-22, -53), (-22, -59))
    layout.carve_hole("region:carnival_midway", BIGTOP)
    layout.add_region("region:carnival_marquee", BIGTOP, role="interior",
                      ceiling_z=COURT - tex(12), floor_z=CARNIVAL_FLOOR - 2048,
                      **material("gallery"),
                      intent={"purpose": "the marquee: open-sided, roofed, and a "
                                         "step up out of the mud",
                              "classification": "OPTIONAL"})
    for index in range(len(BIGTOP)):
        layout.add_connection("connection:marquee_%02d" % index,
                              "region:carnival_midway", "region:carnival_marquee",
                              a1=BIGTOP[index], a2=BIGTOP[(index + 1) % len(BIGTOP)],
                              min_width=512)

    # -- the shooting gallery ------------------------------------------------
    BOOTH = R(4, -68, 16, -64)
    layout.add_region("region:carnival_booth", BOOTH, role="detail", type=614,
                      ceiling_z=COURT - tex(6), floor_z=CARNIVAL_FLOOR - 2048,
                      **material("gallery"),
                      sector_behavior={"rx_id": CH_CARNIVAL, "command": 3,
                                       "busy_time_a": 10, "busy_time_b": 10,
                                       "wait_time_a": 2, "wait_time_b": 2,
                                       "retrigger_a": 1, "retrigger_b": 1,
                                       "shade_wave": TARGET_WAVE,
                                       "amplitude": TARGET_AMPLITUDE,
                                       "shade_frequency": TARGET_FREQUENCY,
                                       "shade_always": 1, "shade_floor": 1,
                                       "shade_ceiling": 1, "shade_walls": 1},
                      intent={"purpose": "shooting gallery: the targets slide, the "
                                         "booth does not",
                              "classification": "OPTIONAL",
                              "interaction": "remote_switch"})
    layout.add_connection("connection:booth_front", "region:carnival_midway",
                          "region:carnival_booth",
                          a1=P(4, -64), a2=P(16, -64), min_width=1536)
    for tag, marker_type, x in (("off", 3, 7.0), ("on", 4, 13.0)):
        layout.add_sprite("carnival_marker_%s" % tag, "region:carnival_booth",
                          x=P(x, -66)[0], y=P(x, -66)[1], z=CARNIVAL_FLOOR - 2048,
                          type=marker_type, picnum=3997, status=10, cstat=32896,
                          x_repeat=64, y_repeat=64, angle=0)
    for tag, tile, x in (("duck_a", TARGET_DUCK, 6.0), ("face_a", TARGET_FACE, 8.5),
                         ("duck_b", TARGET_DUCK, 11.0), ("face_b", TARGET_FACE, 13.5)):
        layout.add_sprite("carnival_target_%s" % tag, "region:carnival_booth",
                          x=P(x, -66)[0], y=P(x, -66)[1],
                          z=CARNIVAL_FLOOR - 2048, seat="floor",
                          type=416, status=4, picnum=tile,
                          cstat=CARRIED | 1 | 128 | 256,
                          x_repeat=48, y_repeat=48, shade=-16)

    # -- the dressing --------------------------------------------------------
    for index, (x, y) in enumerate((
        (-24.0, -44.0), (16.0, -44.0), (-24.0, -66.0), (16.0, -61.0),
        (-14.0, -44.0), (0.0, -46.0), (-4.0, -66.0), (8.0, -60.0),
        (-24.0, -55.0), (16.0, -55.0), (-14.0, -66.0), (2.0, -52.0),
    )):
        layout.add_sprite("carnival_balloon_%02d" % index, "region:carnival_midway",
                          x=P(x, y)[0], y=P(x, y)[1], z=COURT - tex(5), seat="floor",
                          type=416, status=4, picnum=CARNIVAL_BALLOON,
                          cstat=1 | 128 | 256, x_repeat=64, y_repeat=64,
                          pal=BALLOON_PALETTES[index % len(BALLOON_PALETTES)],
                          shade=-24)
    for index, t in enumerate((0.15, 0.35, 0.55, 0.75, 0.92)):
        layout.place_on_wall("carnival_marquee_w_%02d" % index, "region:carnival_midway",
                             a1=P(-26, -42), a2=P(-26, -68), t=t,
                             height_player_heights=1.39, offset_player_widths=0.10,
                             type=0, picnum=CARNIVAL_MARQUEE, cstat=208, pal=14,
                             x_repeat=32, y_repeat=32, shade=-64)
        layout.place_on_wall("carnival_marquee_e_%02d" % index, "region:carnival_midway",
                             a1=P(18, -68), a2=P(18, -42), t=t,
                             height_player_heights=1.39, offset_player_widths=0.10,
                             type=0, picnum=CARNIVAL_MARQUEE, cstat=208, pal=14,
                             x_repeat=32, y_repeat=32, shade=-64)
    layout.place_on_wall("carnival_poster_jojo", "region:carnival_midway",
                         a1=P(-26, -42), a2=P(-26, -68), t=0.45,
                         height_player_heights=0.86, offset_player_widths=0.10,
                         type=417, status=4, picnum=CARNIVAL_POSTER, cstat=208,
                         x_repeat=96, y_repeat=96, shade=-8)
    layout.place_on_wall("carnival_poster_ride", "region:carnival_midway",
                         a1=P(18, -68), a2=P(18, -42), t=0.45,
                         height_player_heights=0.86, offset_player_widths=0.10,
                         type=417, status=4, picnum=CARNIVAL_RIDE_SIGN, cstat=208,
                         x_repeat=96, y_repeat=96, shade=-8)
    for tag, tile, x, y, rep, kind, region in (
        ("lamp_a", CARNIVAL_LAMP, -22.0, -45.0, 64, 0, "region:carnival_midway"),
        ("lamp_b", CARNIVAL_LAMP, 14.0, -45.0, 64, 0, "region:carnival_midway"),
        ("lamp_c", CARNIVAL_LAMP, -22.0, -65.0, 64, 0, "region:carnival_midway"),
        ("lamp_d", CARNIVAL_LAMP, 14.0, -61.0, 64, 0, "region:carnival_midway"),
        ("lamp_e", CARNIVAL_LAMP, -4.0, -44.0, 64, 0, "region:carnival_midway"),
        ("lamp_f", CARNIVAL_LAMP, -14.0, -55.0, 64, 0, "region:carnival_marquee"),
        ("barrel_a", CARNIVAL_BARREL, -20.0, -44.0, 48, 416, "region:carnival_midway"),
        ("barrel_b", CARNIVAL_BARREL, 12.0, -44.0, 48, 416, "region:carnival_midway"),
        ("barrel_c", CARNIVAL_BARREL, 2.0, -66.0, 48, 416, "region:carnival_midway"),
        ("mime_a", CARNIVAL_MIME, -18.0, -53.0, 40, 416, "region:carnival_marquee"),
        ("mime_b", CARNIVAL_MIME, -10.0, -53.0, 40, 416, "region:carnival_marquee"),
        ("mime_c", CARNIVAL_MIME, -14.0, -60.0, 40, 416, "region:carnival_marquee"),
        ("mime_d", CARNIVAL_MIME, 6.0, -50.0, 40, 416, "region:carnival_midway"),
        ("mime_e", CARNIVAL_MIME, -2.0, -60.0, 40, 416, "region:carnival_midway"),
    ):
        floor = (CARNIVAL_FLOOR - 2048 if region.endswith("marquee")
                 else CARNIVAL_FLOOR)
        layout.add_sprite("carnival_%s" % tag, region,
                          x=P(x, y)[0], y=P(x, y)[1], z=floor, seat="floor",
                          type=kind, status=4 if kind else 0, picnum=tile,
                          cstat=1 | 128 | 256, x_repeat=rep, y_repeat=rep,
                          shade=-8 if kind else -64)

    # -- what it says --------------------------------------------------------
    for tag, region, a1, a2, words, height, size, palette, t in (
        ("welcome", "region:carnival_midway", P(-26, -42), P(-26, -68),
         "STEP RIGHT UP", 1.86, 64, "sign", 0.68),
        ("nolaugh", "region:carnival_midway", P(18, -68), P(18, -42),
         "NOBODY LAUGHS TWICE", 1.86, 48, "warning", 0.68),
        ("prize", "region:carnival_booth", P(4, -68), P(16, -68),
         "EVERY ONE A WINNER", 0.86, 40, "rust", 0.5),
        ("mind", "region:carnival_midway", P(-26, -68), P(18, -68),
         "MIND THE GAP", 1.33, 48, "stencil", 0.12),
    ):
        write_on_wall(layout, "sign_carnival_%s" % tag, region, a1=a1, a2=a2,
                      text=words, height_player_heights=height, size=size,
                      palette=palette, t=t)

def make_layout() -> PlanarLayout:
    layout = PlanarLayout(name="monastery-v6", visibility=800,
    ...
    # Hold the vocabulary itself to Build's rule, not just this level's use of
    # it: a material with a 64x400 ceiling is wrong in every level that ever
    # names it, and it should be caught where it is defined.
    complaints = check_materials(layout.flat_tile_sizes)
    if complaints:
        raise ValueError("; ".join(complaints))

    # ---- arrival -----------------------------------------------------------
    layout.add_region("region:ledge", LEDGE, role="start",
                      ceiling_z=LEDGE_CEIL, floor_z=COURT, **material("approach"),
    ...
    layout.add_region("region:courtyard", COURTYARD, role="exterior",
                      ceiling_z=SKY, floor_z=COURT, **material("courtyard"),
    ...
    layout.add_region("region:garden_bed", GARDEN_BED, role="detail",
                      ceiling_z=SKY, floor_z=BED_FLOOR, **material("planting"),
    ...
    layout.add_region("region:chapel_apse", APSE, role="interior",
                      ceiling_z=APSE_CEIL, floor_z=APSE_FLOOR, **material("sanctuary"),
    ...
    layout.add_region("region:chapel_breach_north", R(30, 32, 34, 34), role="gateway",
                      inherit_finish="both",
                      ceiling_z=AISLE_CEIL, floor_z=COURT, **material("arch"),
    ...
    chancel = staircase(
        ...
        base_floor_z=COURT, shade_ramp=(18, 12), **material("sanctuary"),
    ...
    layout.add_region("region:garth", R(20, -14, 40, -2), role="exterior",
                      ceiling_z=SKY, floor_z=GARTH_FLOOR, **material("garth"),
    ...
    layout.add_connection("connection:eastwalk_chapter_n", "region:walk_east",
                          ...
                          min_width=1536,
                          face_cstat=GLASS_CSTAT, face_over_picnum=WINDOW_GLASS)
    ...
    descent = staircase(
        ...
        clear_height=tex(4), base_floor_z=COURT, shade_ramp=(28, 46), **material("crypt_stair"),
        intent={"classification": "MANDATORY"},
    )
    layout.add_region("region:crypt_hall", CRYPT_HALL, role="interior",
                      ceiling_z=CRYPT_CEIL, floor_z=CRYPT_FLOOR, **material("crypt"),
    ...
    layout.add_region("region:crypt_reliquary", RELIQUARY, role="key_branch",
                      ceiling_z=CHAMBER_CEIL, floor_z=CRYPT_FLOOR, **material("charnel"),
    ...
    layout.add_region("region:ossuary_door", R(42, 52, 44, 56), role="doorway", type=617,
                      inherit_finish="both",
                      ceiling_z=OSSUARY_CEIL, floor_z=CRYPT_FLOOR, **material("vault"),
    ...
    layout.add_region("region:crypt_breach", R(25, 49, 29, 50), role="doorway", type=600,
                      inherit_finish="both",
                      door_face=DOOR_FACE,
                      ceiling_z=CRYPT_FLOOR, floor_z=CRYPT_FLOOR, **material("crypt"),
    ...
    ascent = staircase(
        ...
        # Roofed, not open. A staircase's step ceilings track its step floors, so
        # an open flight makes the sky descend a step at a time -- and 94% of the
        # campaign's adjacent open-to-open sector pairs share an identical
        # ceiling z. This one climbs into the gallery arch, and 83% of the
        # campaign's small sectors touching both an open space and a roofed one
        # take the roof.
        clear_height=tex(5), base_floor_z=COURT, shade_ramp=(14, 26), **material("arch"),
        intent={"classification": "MANDATORY"},
    )
    layout.add_region("region:gallery_arch", R(58, 20, 62, 24), role="gateway",
                      inherit_finish="both",
                      ceiling_z=ARCH_CEIL, floor_z=GALLERY_FLOOR, **material("arch"),
    ...
    layout.add_region("region:gallery", GALLERY, role="upper",
                      ceiling_z=GALLERY_CEIL, floor_z=GALLERY_FLOOR, **material("gallery"),
    ...
    # The carnival, off the refectory.
    dark_carnival(layout)
    ...
    descent_home = staircase(
        ...
        total_rise=3 * GAL_STEP, step_rise=GAL_STEP, tread=2 * U,
        clear_height=tex(5), base_floor_z=GALLERY_FLOOR, shade_ramp=(26, 14),
        **material("arch"),
        intent={"classification": "MANDATORY"},
    )
    ...
    layout.add_region("region:exit_hall", EXIT_HALL, role="exit", declared_zero_exit=True,
                      ceiling_z=EXIT_CEIL, floor_z=GALLERY_FLOOR, **material("undercroft"),
    ...
    layout.place_on_wall("crack_crypt", "region:crypt_hall", a1=P(25, 50), a2=P(29, 50), t=0.5,
                         height_player_heights=0.53, offset_player_widths=0.06,
    ...
    def sunk(name, points, purpose, *, under_pool=False):
        layout.add_region(name, points, role="interior",
                          ...
                          **material("flooded_shaft" if under_pool else "flooded"),
    ...
    layout.place_on_wall("sw_ossuary", "region:crypt_cistern", a1=P(32, 66), a2=P(28, 66), t=0.5,
                         ...
                         **SWITCH,
                         ...
                         behavior={"tx_id": CH_SECRET, "command": 3, "trigger_on": 1,
                                   "trigger_push": 1, "data_1": 203})
    # The exit wears the exit's own tile, not the lever every other switch uses.
    #
    # 41 of the campaign's 50 channel-4 switches are tile 318 -- a downward
    # blade, 78x44 -- and that tile appears only 5 times anywhere else in 43
    # maps. It is as near a reserved word as Blood's art gets. This switch was
    # correctly wired to kChannelLevelExitNormal all along and wore 1070, the
    # ordinary lever, which the campaign uses 274 times for opening doors: the
    # level ended without ever saying which switch ended it.
    _exit = exit_switch()
    layout.place_on_wall("sw_exit", "region:exit_hall", a1=P(88, 20), a2=P(88, 28), t=0.5,
                         offset_player_widths=SWITCH_OFFSET,
                         **{k: v for k, v in _exit.items()
                            if k != "height_player_heights"},
                         height_player_heights=_exit["height_player_heights"])
    ...
    #
    # Blood has no text primitive. It has an alphabet in the ART -- 3808 to 3833,
    # A to Z, monospace at 8 by 11 -- and its levels spell things out one sprite
    # per letter. E1M4 is full of it: STORAGE over a warehouse door, RATDOGS on a
    # crate, FORTUNES, SEEING IS BELIEVING. E1M2 marks TRACK A, E2M1 names a ship
    # HMS VICTOR, E3M2 has FEINMAN MEATS and a LOADING bay.
    #
    # The colour is the part with no equivalent elsewhere in the format: the same
    # 26 tiles are recoloured by palette lookup, and the campaign paints its
    # letters in eight different palettes. A sign's colour is a property of the
    # sign rather than of the tile it is made from.
    #
    # See `bloodmap.lettering` for the conventions -- cstat 208, square repeats,
    # and a pitch of 1.45 times a letter's drawn width, which is what the
    # campaign's 36 words agree on across every size it writes at.
    for tag, region, a1, a2, words, height, size, palette in (
        ("cabal", "region:courtyard", P(44, 32), P(44, 14), "CABAL", 1.66, 136, "warning"),
        ("memento", "region:ossuary", P(52, 52), P(52, 58), "MEMENTO MORI", 0.73, 48, "stencil"),
        ("catacomb", "region:exit_hall", P(78, 18), P(86, 18), "CATACOMB", 1.0, 64, "sign"),
        ("stores", "region:refectory", P(4, -4), P(4, -14), "STORES", 1.0, 64, "rust"),
        ("sanctum", "region:gallery", P(74, 42), P(64, 42), "SANCTUM", 1.33, 96, "cold"),
    ):
        write_on_wall(layout, "sign_%s" % tag, region, a1=a1, a2=a2, text=words,
                      height_player_heights=height, size=size, palette=palette)
    ...
    recess(layout, "recess:crypt_niche",
           ...
           depth=int(1.5 * U), ceiling_drop=tex(2), **material("charnel"),
    ...
    recess(layout, "recess:gallery_niche",
           ...
           depth=int(1.5 * U), ceiling_drop=tex(6), **material("gallery"),
    ...
    recess(layout, "recess:exit_niche",
           ...
           depth=int(1.5 * U), ceiling_drop=tex(4), **material("undercroft"),
...
def _bind_rotation_axis(level) -> None:
    ...
    for wall in level.walls:
        ...
        if int(fields["picnum"]) == GIB_FACE and int(fields["next_sector"]) >= 0:
            fields["type"] = 511
    # And the glass. A pane you cannot break is a pane the player will spend a
    # magazine on before deciding the level is broken.
    for wall in level.walls:
        fields = wall["fields"]
        if int(fields["over_picnum"]) == WINDOW_GLASS and int(fields["next_sector"]) >= 0:
            fields["type"] = 511
...
def _decorate(layout: PlanarLayout) -> None:
    ...
    layout.place_on_wall("dec_ledge_torch", "region:ledge", a1=P(-10, 24), a2=P(-10, 20),
                         t=0.5, height_player_heights=0.63, offset_player_widths=0.10, **torch(1.5))
    layout.place_on_wall("dec_tunnel_torch", "region:gate_tunnel", a1=P(-2, 21), a2=P(6, 21),
                         t=0.7, height_player_heights=0.5, offset_player_widths=0.10, **torch(1.0))
    layout.place_on_wall("dec_gate_emblem", "region:gatehouse", a1=P(10, 27), a2=P(6, 24),
                         t=0.5, height_player_heights=0.86, offset_player_widths=0.12, **emblem(2540, 1.0))
    layout.place_on_wall("dec_gate_grille", "region:gatehouse", a1=P(6, 20), a2=P(10, 17),
                         t=0.5, height_player_heights=0.63, offset_player_widths=0.12, **grille(1.6))
    ...
    for name, t_value in (("south", 0.30), ("north", 0.70)):
        layout.place_on_wall(f"dec_chapel_torch_{name}", "region:courtyard",
                             ...
                             height_player_heights=0.63, offset_player_widths=0.10, **torch(1.5))
    for name, t_value in (("south", 0.06), ("north", 0.94)):
        layout.place_on_wall(f"dec_chapel_vine_{name}", "region:courtyard",
                             a1=P(24, 14), a2=P(24, 32), t=t_value,
                             height_player_heights=0.02, offset_player_widths=0.12, **shrub())
    layout.place_on_wall("dec_court_emblem", "region:courtyard", a1=P(44, 32), a2=P(44, 14),
                         t=0.5, height_player_heights=0.86, offset_player_widths=0.12, **emblem(2545, 1.2))
    ...
    for name, local in (("west", (0.32, 0.5)), ("east", (0.68, 0.5))):
        layout.place_on_ceiling(f"dec_nave_lamp_{name}", "region:chapel_nave",
                                local=local, height_player_heights=0.27, **lamp(1.3))
    for name, t_value in (("south", 0.16), ("north", 0.84)):
        layout.place_on_wall(f"dec_nave_sconce_{name}", "region:chapel_nave",
                             ...
                             height_player_heights=0.76, offset_player_widths=0.12, **sconce(0.9))
    ...
    layout.place_on_wall("dec_nave_grille_s", "region:chapel_nave", a1=P(28, 18), a2=P(36, 18),
                         t=0.5, height_player_heights=1.06, offset_player_widths=0.12,
    ...
    layout.place_on_wall("dec_aisle_n_plank", "region:chapel_aisle_north",
                         ...
                         height_player_heights=0.53, offset_player_widths=0.10,
    ...
    for name, t_value in (("west", 0.15), ("east", 0.85)):
        layout.place_on_wall(f"dec_crypt_grille_{name}", "region:crypt_hall",
                             ...
                             height_player_heights=0.63, offset_player_widths=0.12, **grille(1.5))
    ...
    layout.place_on_wall("dec_crypt_emblem", "region:crypt_hall", a1=P(40, 60), a2=P(25, 60),
                         t=0.33, height_player_heights=0.73, offset_player_widths=0.12,
                         **emblem(2545, 1.0))
    ...
    layout.place_on_wall("dec_reliquary_emblem", "region:crypt_reliquary",
                         ...
                         height_player_heights=0.53, offset_player_widths=0.12, **emblem(2545, 0.8))
    ...
    layout.place_on_wall("dec_reliquary_sconce", "region:crypt_reliquary",
                         ...
                         height_player_heights=0.5, offset_player_widths=0.12, **sconce(1.3))
    ...
    layout.place_on_wall("dec_cistern_disc", "region:crypt_cistern", a1=P(34, 60), a2=P(34, 64),
                         t=0.5, height_player_heights=0.53, offset_player_widths=0.12, **emblem(915, 1.4))
    layout.place_on_wall("dec_ossuary_emblem", "region:ossuary", a1=P(52, 52), a2=P(52, 58),
                         t=0.5, height_player_heights=0.53, offset_player_widths=0.12, **emblem(2545, 0.8))
    layout.place_on_wall("dec_ossuary_sconce", "region:ossuary", a1=P(50, 60), a2=P(44, 60),
                         t=0.5, height_player_heights=0.5, offset_player_widths=0.12, **sconce(1.3))
    ...
    for tag, a1, a2, t in (("south_a", (46, 2), (14, 2), 0.25),
        ...
        layout.place_on_wall(f"dec_walk_torch_{tag}", f"region:walk_{tag.split('_')[0]}",
                             a1=P(*a1), a2=P(*a2), t=t,
                             height_player_heights=0.63, offset_player_widths=0.10,
                             **torch(1.5))
    ...
    for name, t_value in (("north", 0.3), ("south", 0.7)):
        layout.place_on_wall(f"dec_gallery_torch_{name}", "region:gallery",
                             ...
                             height_player_heights=0.73, offset_player_widths=0.10, **torch(1.6))
    layout.place_on_wall("dec_gallery_plank", "region:gallery", a1=P(62, 40), a2=P(62, 12),
                         t=0.12, height_player_heights=0.53, offset_player_widths=0.10,
    ...
    for name, local in (("west", (0.25, 0.5)), ("east", (0.75, 0.5))):
        layout.place_on_ceiling(f"dec_gallery_lamp_{name}", "region:gallery",
                                local=local, height_player_heights=0.3, **lamp(2.2))
    layout.place_on_wall("dec_gallery_grille", "region:gallery", a1=P(64, 10), a2=P(73, 10),
                         t=0.5, height_player_heights=1.16, offset_player_widths=0.12, **grille(2.4))
    layout.place_on_wall("dec_arch_emblem", "region:gallery_arch", a1=P(58, 20), a2=P(62, 20),
                         t=0.5, height_player_heights=0.66, offset_player_widths=0.12, **emblem(2540, 0.9))
    ...
    for name, t_value in (("north", 0.25), ("south", 0.75)):
        layout.place_on_wall(f"dec_exit_sconce_{name}", "region:exit_hall",
                             ...
                             height_player_heights=0.73, offset_player_widths=0.12, **sconce(0.9))
    layout.place_on_ceiling("dec_exit_light", "region:exit_hall", local=(0.5, 0.5),
                            height_player_heights=0.17, **disc(0.7))
    layout.place_on_wall("dec_exit_chain", "region:exit_hall", a1=P(78, 18), a2=P(86, 18),
                         t=0.5, height_player_heights=0.86, offset_player_widths=0.12, **chain(2.6))
    ...
    return AuthoredIntent(
        ...
        assemblies=(
            AuthoredAssembly(
                ...
                material_vocabulary={"material": 'approach'},
            ),
            AuthoredAssembly(
                ...
                material_vocabulary={"material": 'courtyard'},
            ...
            AuthoredAssembly(
                ...
                material_vocabulary={"material": 'nave'},
            ...
            AuthoredAssembly(
                ...
                material_vocabulary={"material": 'crypt'},
            ),
            AuthoredAssembly(
            ...
            AuthoredAssembly(
                ...
                material_vocabulary={"material": 'gallery'},
            ),
            AuthoredAssembly(
                ...
                material_vocabulary={"material": 'undercroft'},
            ),
        ),
        transitions=(
            AuthoredTransition(
        ...
        material_vocabulary={
            "note": "named, not numbered: every surface in this level comes from "
                    "bloodmap.surfaces, which carries the tiles, the tile each "
                    "material dresses its own openings in, and whether its "
                    "ceiling is the sky -- and which refuses on import any flat "
                    "tile Build cannot sample",
...
def candidate() -> Candidate:
    return Candidate(
        iteration_id="v6",
        module="projects/reasoned-authoring-v1/level/candidate_v6.py",
        factory=make_layout,
        intent=intent(),
        probes=probes(),
        ...
        parent="v5",
        declared_changes=(
            "surfaces are named, not numbered: every region takes a material from "
            "`bloodmap.surfaces` -- 27 of them -- which carries the three tiles, "
            "the tile that material dresses its own openings in, and whether its "
            "ceiling is the sky. Twenty local `*_TILES` dicts and every loose "
            "picnum in the level are gone",
            "the sky is part of the material rather than a flag beside it. Three "
            "doorways had inherited a neighbour's 64x400 sky panel without the "
            "parallax bit and were drawing it as an ordinary ceiling, where Build "
            "samples the largest power of two not greater than each side -- 64x256 "
            "-- and never draws the rest. The compiler now refuses any flat tile "
            "with a non-power-of-two side; the campaign obeys that rule on 26,376 "
            "of 26,383 surfaces",
            "things are named, not numbered: `furnish(\"torch\")` in place of "
            "`decor(506, 128, 1.5, shade=-128)`, with the mounting taken from the "
            "tile's canonical cstat and the size from the median the campaign "
            "draws it at. A tile is declared dry or wet, because 664 and 660 "
            "appear in 82 and 142 campaign sectors and every one is under water",
            "shade is derived and no longer written down. Fifty-nine `shades(N)` "
            "calls and fourteen `sky_shades(N)` are gone: roofed 32, open 16, a "
            "hall seven brighter, which is what accounts for the campaign's own "
            "13,649 playable sectors",
            "light is cast rather than assumed. XMapEdit's light bomb, transcribed "
            "from edit3d.cpp: rays out of every flame, hitscanned through the map, "
            "energy divided by the area of what they land on, reflected twice, sky "
            "absorbing. Its two free parameters are fitted to the campaign's "
            "measured falloff rather than left at the editor's defaults, which "
            "produce no distance falloff at all",
            "the chancel's three invisible barriers are gone. Blood blocks 2,272 "
            "two-sided walls, at a median floor difference of 4.00 player heights "
            "and a q1 of 1.09; these sat at 0.36 with nothing drawn on them, so "
            "the geometry said step and the wall said no",
            "the cistern looks like what it does. It has damaged and dragged the "
            "player since it was built, wearing plain dark stone; it now wears the "
            "campaign's commonest panned floor, six frames of green sludge",
        ),
    )
