"""Cliffside monastery, iteration 4: structures, not sectors.

This iteration is the first to be driven by the structure layer the decompiler
gained after E2M3 was taken apart.  Three things the packet had never been able
to say about v3, and what was done about each:

1. ZERO VERTICAL RELATIONSHIPS.  ``structure_recovery`` found no overlook and no
   pit anywhere in a level that spans three storeys.  Every height change in v3
   is a stair you walk up; nothing is ever *seen* from another height.  All 42
   campaign maps contain overlooks and the median map has 113.
     - the garden bed becomes a raised planter (6144 above the courtyard, five
       times what the player can step over), so the courtyard gains eight
       one-way vertical relationships instead of a shallow tray;
     - the chancel is raised from 2048 to 6144 above the nave and reached by a
       two-tread stair beside a direct opening, so the chapel gains an overlook
       and the chancel finally stands over the nave rather than on a kerb.
   No pit was added.  A pit by this measure is somewhere the player cannot climb
   out of, and nowhere in a monastery should trap them; that is an argued
   exception, not an oversight.

2. THE STAIR RISE WAS OFF-VOCABULARY.  The corpus uses 4096, 2048 and 3072 for
   1277 of its 1283 stair rises, and 4096 twice as often as any other.  v3's
   gallery ascent used 3840, a number with no support at all.  It is now 4096.
   The crypt keeps 3072, which the corpus does use.

3. STAIRS AND NICHES WERE HAND-ROLLED.  Every stair in v0-v3 was a copied loop
   that tracked its own previous edge, and getting the winding of that edge
   backwards is what produced most of the pilot's compile failures.  All four
   stairs are now ``bloodmap.vocabulary.staircase`` calls and the new niches are
   ``recess`` calls, sized on the corpus profile: floor flush with the host and
   the ceiling dropped, at a few percent of the host's area.

Scale, materials and the shade structure that carries the perceptual thresholds
are inherited from v3.  Shape is deliberately left alone: the chamfer and
orientation findings are the next iteration's subject, not this one's.
"""
...
APSE = poly((41, 20), (42, 20), (43, 21), (43, 25), (42, 26), (41, 26),
            (38, 26), (38, 23), (41, 23))
...
def make_layout() -> PlanarLayout:
    ...
    descent = staircase(
        ...
        clear_height=4 * PH, base_floor_z=COURT, shade_ramp=(28, 46), **CRYPT_TILES,
        intent={"classification": "MANDATORY"},
    )
    ...
    recess(layout, "recess:crypt_niche",
           ...
           depth=int(1.5 * U), ceiling_drop=2 * PH, **CRYPT_TILES, **shades(36),
...
def _decorate(layout: PlanarLayout) -> None:
    ...
    for name, t_value in (("south", 0.25), ("north", 0.75)):
        layout.place_on_wall(f"dec_apse_sconce_{name}", "region:chapel_apse",
                             a1=P(42, 20), a2=P(43, 21), t=t_value,
...
def candidate() -> Candidate:
    return Candidate(
        iteration_id="v4",
        module="projects/reasoned-authoring-v1/level/candidate_v4.py",
        factory=make_layout,
        intent=intent(),
        probes=probes(),
        ...
        parent="v3",
        declared_changes=(
            "the garden bed raised 6144 above the courtyard instead of sunk 2048, and the "
            "chancel raised 6144 above the nave instead of 2048, so the level finally has "
            "one-way vertical relationships; v3 had none anywhere",
            "a three-tread chancel stair beside a direct 6144 opening, so the chancel is "
            "reached without closing the view back down into the nave",
            "the gallery ascent rise changed from 3840 to 4096, the rise the corpus uses "
            "for 771 of its 1283 stair rises",
            "all four stairs written as bloodmap.vocabulary.staircase calls instead of "
            "hand-rolled loops that tracked their own previous edge",
            "three wall niches written as bloodmap.vocabulary.recess calls, sized on the "
            "corpus profile: flush floor, lowered ceiling, a few percent of the host",
            "a raised tomb block in the crypt hall: a third vertical relationship, and "
            "something for the level's second-largest interior to be about",
        ),
    )
