...
`bloodmap.read_surfaces` groups this map's wall records into the surfaces one
...
from _common import MAP_NAME, art_sizes, level, write
...
def _review(level_ir, result, owners, stairs) -> dict:
    ...
    map: the parts of this map where every wall carries its own projection.
    """
    tree = Tree(len(level_ir.sectors), f"{MAP_NAME} -- surfaces of more than one record")
    ...
    #: Stairs are STRUCTURES, not mechanisms: a helix carries no sector
    ...
    questions = [
        {"node": biggest.surface_id,
         "question": ("the largest recovered surface here spans "
                      ...
                      "one frame per RUN, or per FLAT FACE? Measured over "
                      "the 43 campaign maps the answer is the run: 93.7% "
                      "of collinear solid-solid joins continue, 67.9% of "
                      "bends and 19.1% of reflex corners."),
         "recommended_default": ("keep the run, and add nothing: "
                                 ...
                                 "campaign, which is now measured: see "
                                 "reports/campaign-census-2026-09-03.md"),
    ...
    return write_pack(2, tree, f"{MAP_NAME} layer 2: surfaces and frames", questions)
...
def main() -> int:
    ...
    #: The corner rule, read off this map rather than assumed: where does a
    ...
    print(f"{MAP_NAME} surfaces: {stats['surfaces']} recovered, "
